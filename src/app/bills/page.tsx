...
type ActiveTab = 'payable' | 'receivable'
...
export default function BillsPage() {
  ...
  const [activeTab, setActiveTab] = useState<ActiveTab>('payable')
  ...
  }, [activeTab, dateRange.end, dateRange.start, statusFilter, userId])
  ...
  }, [userId, fetchProfileCurrency])

  useEffect(() => {
    if (!authLoading && userId) {
      fetchBills()
    }
  }, [authLoading, userId, fetchBills])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.party_name.trim()) {
      toast({
        title: 'Nome inválido',
        description: 'Informe o nome do fornecedor ou cliente',
        variant: 'destructive',
      })
      return
    }
    ...
    try {
      ...
      if (activeTab === 'payable') {
        billData.supplier_name = formData.party_name.trim()
      } else {
        billData.client_name = formData.party_name.trim()
      }
      ...
      toast({
        title: 'Sucesso',
        description: editingId ? 'Conta atualizada com sucesso' : 'Conta adicionada com sucesso',
      })
    } catch (error: any) {
  ...
  const handleDelete = async (id: string) => {
    if (!supabase || !window.confirm('Tem certeza que deseja excluir esta conta?')) return
    ...
    <div className="min-h-screen bg-black">
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          ...
          <div className="bg-gray-900 rounded-2xl border border-gray-800 p-6">
            <p className="text-gray-400 text-sm mb-2">Vencidas</p>
            <p className="text-3xl font-bold text-orange-500">{overdueBills.length}</p>
          </div>

          <div className="bg-gray-900 rounded-2xl border border-gray-800 p-6">
            <p className="text-gray-400 text-sm mb-2">Pagas</p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  <Label className="text-gray-300">
                    {activeTab === 'payable' ? 'Fornecedor / Destinatário' : 'Cliente / Pagador'}
                  </Label>
                  <Input
                    placeholder={
                      activeTab === 'payable'
                        ? 'Ex: Conta de luz, João Silva'
                        : 'Ex: Empresa X'
                    }
                ...
                <div className="flex-1 min-w-[240px]">
                  ...
                  {bill.is_recurring && (
                    <p className="text-amber-600 text-sm">
                      Recorrente: {bill.recurrence_interval || 'monthly'}
                    </p>
                  )}
                </div>
